...
export function AnalyticsCharts({
  ...
  // Use monthly data as fallback if daily report has no data
  const hasValidData =
    report &&
  ...
  const effectiveReport = hasValidData ? report : monthlyReport;

  const categoryData = effectiveReport
    ? Object.entries(effectiveReport.category_breakdown || {}).map(
        ([name, value]) => ({
          name: name
            .replace(/_/g, " ")
            .toLowerCase()
  ...
  const netData = effectiveReport
    ? [
        {
          name: "Income",
          value: Number(effectiveReport.total_income || 0),
          fill: "rgb(0,255,178)",
        },
        {
          name: "Expenses",
          value: Number(effectiveReport.total_expenses || 0),
          fill: "#F59E0B",
        },
      ]
    : [];
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .recharts-sector:hover,
        .recharts-rectangle:hover {
          filter: drop-shadow(0 0 12px rgba(0, 255, 178, 0.6)) brightness(1.15)
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="bg-gradient-to-br from-[rgba(255,255,255,0.08)] to-[rgba(255,255,255,0.02)] rounded-2xl border border-white/10 p-6">
          ...
          <div className="h-80">
            {netData.length > 0 && netData.some((item) => item.value !== 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={netData}
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-sm text-white/40">
                    Add income and expense transactions to see the comparison
                  </p>
                </div>
              </div>
            )}
