...
import { Calendar, DollarSign, Tag, FileText, Plus, Loader2, Sparkles } from "lucide-react"

const categories = [
  { value: "FOOD_DINING", label: "Food & Dining", color: "from-orange-500 to-amber-500" },
  { value: "GROCERIES", label: "Groceries", color: "from-green-500 to-emerald-500" },
  { value: "TRANSPORTATION", label: "Transportation", color: "from-blue-500 to-indigo-500" },
  { value: "SHOPPING", label: "Shopping", color: "from-purple-500 to-pink-500" },
  { value: "ENTERTAINMENT", label: "Entertainment", color: "from-pink-500 to-rose-500" },
  { value: "BILLS_UTILITIES", label: "Bills & Utilities", color: "from-yellow-500 to-orange-500" },
  { value: "HEALTHCARE", label: "Healthcare", color: "from-blue-500 to-purple-500" },
  ...
  { value: "FITNESS", label: "Fitness", color: "from-emerald-500 to-green-500" },
  { value: "PERSONAL_CARE", label: "Personal Care", color: "from-teal-500 to-cyan-500" },
  { value: "GIFTS_DONATIONS", label: "Gifts & Donations", color: "from-rose-500 to-pink-500" },
  { value: "BUSINESS", label: "Business", color: "from-slate-500 to-gray-500" },
  { value: "MISCELLANEOUS", label: "Miscellaneous", color: "from-gray-500 to-slate-500" },
]
...
export function TransactionForm({ trigger }: TransactionFormProps) {
  ...
  useEffect(() => {
    if (type === "expense" && description.trim().length > 3 && amount && parseFloat(amount) > 0) {
      const timeoutId = setTimeout(async () => {
        ...
        try {
          ...
          const response = await apiPost<CategorizeResponse>(`/transactions/categorize?${params.toString()}`, null, true)
          
          if (response?.suggested_category) {
            setCategory(response.suggested_category)
            const categoryLabel = categories.find(c => c.value === response.suggested_category)?.label
            toast({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: `${type === "income" ? "Income" : "Expense"} transaction added successfully`,
      })
